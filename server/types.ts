...
export interface Farmer {
  ...
  dob: string;
  region: string; // State (e.g. Kaduna, Ogun, Kano, Benue, Niger, Oyo)
  district: string; // LGA (Local Government Area)
  village: string;
  ...
  registeredAt: string;
  pin?: string; // 4-digit PIN for self-service portal
  notes?: string;
  assets?: FarmerAsset[];
}
...
export interface Loan {
  ...
  interestRate: number; // in percentage, e.g. 5.5%
...
export interface InfrastructureAsset {
  ...
  estimatedValueUsd: number; // Kept for backwards compatibility
  estimatedValueNaira: number;
...
export interface SystemStats {
  ...
  totalCapitalDisbursedUsd: number; // Kept for backwards compat
